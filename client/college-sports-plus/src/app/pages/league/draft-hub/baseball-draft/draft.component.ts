...
@Component({
  ...
  selector: 'baseball-draft',
  templateUrl: 'draft.component.html',
})
export class BaseballDraftComponent implements OnInit {
  ...
  playerDialogPlayer: WritableSignal<BaseballDraftPlayerModel> = signal(
  ...
  draftPlayers: Array<BaseballDraftPlayerModel> = [];
  ...
  ngOnInit() {
    ...
    const draftKey = localStorage.getItem('bsbdk');
  ...
  showDialog(player: LeagueAthleteModel): void {
    const p: BaseballDraftPlayerModel =
  ...
  onCreateDraft(): void {
    ...
    this.fastApiService
      .createBaseballDraft(createPickOrderData)
    ...
    this.draftWebSocketService
      ...
      .subscribe({
        next: (message) => {
          ...
          localStorage.setItem('bsbdk', `${joinKey}-${this.activeLeague?.ID}`);
        },
        error: (err) => {
          console.error(err);
  ...
  private draftOrderUpdate(message: any): void {
    ...
    //   return p;
    // });
    let draftPlayers: Array<BaseballDraftPlayerModel> = [];
    ...
    this._myTeam.value.forEach((myPlayer) => {
      athletes = athletes?.filter((athlete) => {
        if (
          athlete.Position == BaseballPositionEnum.INF ||
          athlete.Position == BaseballPositionEnum.OF
        ) {
          return true;
        } else {
    ...
    this._myTeam.value.forEach((athlete) => {
      if (athlete.Position == BaseballPositionEnum.OF) {
        OFcount--;
      }
      if (athlete.Position == BaseballPositionEnum.INF) {
        INFcount--;
      }
    });
    if (INFcount <= 0) {
      athletes = athletes?.filter(
        (athlete) => athlete.Position !== BaseballPositionEnum.INF
      );
    ...
    if (OFcount <= 0) {
      athletes = athletes?.filter(
        (athlete) => athlete.Position !== BaseballPositionEnum.OF
      );
      q = this._queue.value?.filter(
        (athlete) => athlete.Position !== BaseballPositionEnum.OF
      );
