...
@Component({
  ...
  selector: 'football-draft',
  templateUrl: 'draft.component.html',
})
export class FootballDraftComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    const draftKey = localStorage.getItem('fbdk');
  ...
  onCreateDraft(): void {
    ...
    this.fastApiService
      .createFootballDraft(createPickOrderData)
    ...
    this.draftWebSocketService
      ...
      .subscribe({
        next: (message) => {
          ...
          localStorage.setItem('fbdk', `${joinKey}-${this.activeLeague?.ID}`);
        },
        error: (err) => {
          console.error(err);
