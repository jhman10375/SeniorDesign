...
@Component({
  ...
  selector: 'soccer-draft',
  templateUrl: 'draft.component.html',
})
export class SoccerDraftComponent implements OnInit {
  ...
  playerDialogPlayer: WritableSignal<SoccerDraftPlayerModel> = signal(
  ...
  draftPlayers: Array<SoccerDraftPlayerModel> = [];
  ...
  ngOnInit() {
    ...
    const draftKey = localStorage.getItem('sccdk');
  ...
  showDialog(player: LeagueAthleteModel): void {
    const p: SoccerDraftPlayerModel =
  ...
  onCreateDraft(): void {
    ...
    this.fastApiService
      .createSoccerDraft(createPickOrderData)
    ...
    this.draftWebSocketService
      ...
      .subscribe({
        next: (message) => {
          ...
          localStorage.setItem('sccdk', `${joinKey}-${this.activeLeague?.ID}`);
        },
        error: (err) => {
          console.error(err);
  ...
  private draftOrderUpdate(message: any): void {
    ...
    //   return p;
    // });
    let draftPlayers: Array<SoccerDraftPlayerModel> = [];
    ...
    this._myTeam.value.forEach((myPlayer) => {
      athletes = athletes?.filter((athlete) => {
        if (
          athlete.Position == SoccerPositionEnum.F ||
          athlete.Position == SoccerPositionEnum.MD ||
          athlete.Position == SoccerPositionEnum.D
        ) {
          return true;
        } else {
    ...
    this._myTeam.value.forEach((athlete) => {
      if (athlete.Position == SoccerPositionEnum.F) {
        Fcount--;
      }
      if (athlete.Position == SoccerPositionEnum.MD) {
        MDcount--;
      }
      if (athlete.Position == SoccerPositionEnum.D) {
        Dcount--;
      }
    });
    if (Fcount <= 0) {
      athletes = athletes?.filter(
        (athlete) => athlete.Position !== SoccerPositionEnum.F
      );
    ...
    if (MDcount <= 0) {
      athletes = athletes?.filter(
        (athlete) => athlete.Position !== SoccerPositionEnum.MD
      );
      q = this._queue.value?.filter(
        (athlete) => athlete.Position !== SoccerPositionEnum.MD
      );
    ...
    if (Dcount <= 0) {
      athletes = athletes?.filter(
        (athlete) => athlete.Position !== SoccerPositionEnum.D
      );
      q = this._queue.value?.filter(
        (athlete) => athlete.Position !== SoccerPositionEnum.D
      );
