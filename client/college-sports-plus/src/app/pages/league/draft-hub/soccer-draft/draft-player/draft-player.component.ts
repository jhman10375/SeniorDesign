...
import { BaseballDraftPlayerModel } from '../../baseball-draft/models/sport-player/baseball.model';
import { BasketballDraftPlayerModel } from '../../basketball-draft/models/sport-player/basketball.model';
import { FootballDraftPlayerModel } from '../../football-draft/models/sport-player/football.model';
...
export class DraftPlayerComponent implements OnInit {
  ...
  @Input() set baseballDraftPlayer(v: BaseballDraftPlayerModel) {
    this.isLoading.set(true);
    if (v && v.Athlete && v.Athlete.AthleteID.length > 0) {
      setTimeout(() => {
        this.baseballPlayerStats = v.Stats;
        this.isLoading.set(false);
      }, 500);
    }
  }

  @Input() set basketballDraftPlayer(v: BasketballDraftPlayerModel) {
    this.isLoading.set(true);
    if (v && v.Athlete && v.Athlete.AthleteID.length > 0) {
      setTimeout(() => {
        this.basketballPlayerStats = v.Stats;
        this.isLoading.set(false);
      }, 500);
    }
  }

  @Input() set soccerDraftPlayer(v: SoccerDraftPlayerModel) {
    this.isLoading.set(true);
    if (v && v.Athlete && v.Athlete.AthleteID.length > 0) {
      setTimeout(() => {
        this.soccerPlayerStats = v.Stats;
        this.isLoading.set(false);
      }, 500);
    }
  }
